...
class ChatHTMLGenerator(BaseHTMLGenerator):
    ...
    def _build_render_context(self):
        ...
        if self.thread.thread_name:
            ...
            chat_name = (
                ", ".join([p[0] for p in others]) if others else f"Conversa {self.thread.thread_id}"
            )
        ...
        participants_html = "".join(
            [f'<span class="participant">{p[0]} <small>({p[1]})</small></span>' for p in others]
        )
    ...
    def _iter_messages_html(self):
        ...
        for msg in self.thread.messages:
            ...
            is_grouped = (
                ...
                and msg.sent is not None
                and prev_sent is not None
                and (msg.sent - prev_sent).total_seconds() <= 300
            )

            yield self._generate_message(msg, is_sent, is_grouped)
            prev_author = msg.author
            prev_sent = msg.sent

    def _generate_message(self, msg: Message, is_sent: bool, is_grouped: bool = False) -> str:
        ...
        return f"""<div class="message {msg_class}{grouped_class}">
...
<head>
    ...
    <style>
        *{{margin:0;padding:0;box-sizing:border-box}}
        ...
        .message.sent .message-bubble{{background:linear-gradient(135deg,{COLOR_SENT_START},{COLOR_SENT_END});color:#fff;border-bottom-right-radius:4px;box-shadow:0 2px 8px rgba(0,0,0,0.2)}}
        .message.received .message-bubble{{background:linear-gradient(135deg,{COLOR_RECEIVED_START},{COLOR_RECEIVED_END});color:#fff;border-bottom-left-radius:4px;box-shadow:0 2px 8px rgba(0,0,0,0.15)}}
        .color-legend{{margin-top:12px;font-size:12px;color:#666;display:flex;gap:16px;flex-wrap:wrap}}
        .color-legend span{{display:inline-flex;align-items:center;gap:6px}}
        .color-legend .swatch{{width:12px;height:12px;border-radius:3px;display:inline-block}}
        .color-legend .swatch-sent{{background:linear-gradient(135deg,{COLOR_SENT_START},{COLOR_SENT_END})}}
        .color-legend .swatch-received{{background:linear-gradient(135deg,{COLOR_RECEIVED_START},{COLOR_RECEIVED_END})}}
        .message.grouped{{margin-top:2px}}
        .message.grouped .message-author{{display:none}}
        .message-author{{display:table;width:fit-content;padding:2px 7px;border-radius:8px;background:rgba(255,255,255,0.9);font-size:12px;font-weight:700;margin-bottom:5px}}
        ...
        .message.received .message-content{{color:#fff}}
        .message-time{{font-size:11px;color:rgba(255,255,255,0.7);margin-top:5px;text-align:right}}
        .message.sent .message-time{{color:rgba(255,255,255,0.7)}}
        .attachment{{margin-top:10px;border-radius:12px;overflow:hidden}}
        .audio-container{{display:flex;flex-direction:column;gap:6px;padding:8px;background:rgba(255,255,255,0.2);border-radius:10px;min-width:280px}}
        .message.sent .audio-container{{background:rgba(255,255,255,0.2)}}
        .audio-container audio{{width:100%;height:36px}}
        .audio-transcription{{margin-top:8px;padding:10px 12px;background:rgba(255,255,255,0.15);border-radius:8px;font-size:13px;line-height:1.5}}
        .message.sent .audio-transcription{{background:rgba(255,255,255,0.15)}}
        .transcription-label{{font-weight:600;color:rgba(255,255,255,0.8);display:block;margin-bottom:4px;font-size:11px}}
        .message.sent .transcription-label{{color:rgba(255,255,255,0.8)}}
        .transcription-text{{color:rgba(255,255,255,0.9)}}
        .message.sent .transcription-text{{color:rgba(255,255,255,0.9)}}
        .audio-speed{{display:flex;gap:5px;justify-content:center}}
        .audio-speed button{{padding:4px 10px;border:none;border-radius:12px;background:rgba(255,255,255,0.3);color:#fff;font-size:11px;cursor:pointer;transition:all .2s}}
        .audio-speed button:hover{{background:rgba(255,255,255,0.4);color:#fff}}
        ...
        .attachment-file{{display:flex;align-items:center;gap:10px;padding:10px 15px;background:rgba(255,255,255,0.2);border-radius:10px}}
        .message.sent .attachment-file{{background:rgba(255,255,255,0.2)}}
        .attachment-file a{{color:#fff;text-decoration:none}}
        .message.sent .attachment-file a{{color:#fff}}
        .attachment-filename{{font-size:10px;color:rgba(255,255,255,0.7);margin-top:6px;padding:4px 8px;word-break:break-word;text-align:center;line-height:1.3;background:rgba(255,255,255,0.1);border-radius:4px}}
        .message.sent .attachment-filename{{color:rgba(255,255,255,0.7);background:rgba(255,255,255,0.1)}}
        .call-info{{display:flex;align-items:center;gap:10px;padding:10px 15px;background:rgba(255,255,255,0.2);border-radius:10px}}
        ...
        .share-link{{margin-top:8px;padding:10px 15px;background:rgba(255,255,255,0.2);border-radius:10px}}
        .message.sent .share-link{{background:rgba(255,255,255,0.2)}}
        .share-link a{{color:#fff;text-decoration:none;font-size:13px;word-break:break-all}}
        .message.sent .share-link a{{color:#fff}}
        .share-link a:hover{{text-decoration:underline}}
        .share-url{{font-size:10px;color:rgba(255,255,255,0.6);margin-top:6px;word-break:break-all;line-height:1.3;padding:4px 6px;background:rgba(255,255,255,0.1);border-radius:4px;font-family:monospace}}
        .message.sent .share-url{{color:rgba(255,255,255,0.6);background:rgba(255,255,255,0.1)}}
        .removed-message{{font-style:italic;color:rgba(255,255,255,0.5)}}
    ...
    <div class="container">
        <div class="chat-header">
            ...
            <div class="color-legend" aria-label="Legenda de cores das mensagens">
                <span><i class="swatch swatch-sent" aria-hidden="true"></i> Alvo (enviadas)</span>
                <span><i class="swatch swatch-received" aria-hidden="true"></i> Interlocutores (recebidas)</span>
            </div>
        </div>
