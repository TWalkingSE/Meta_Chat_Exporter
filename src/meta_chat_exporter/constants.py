...
# ============================================================================
# CORES DOS BALÕES (HTML exportado)
# ============================================================================
# Fonte única para generators_single e templates_all (evita drift visual).
# Sent = alvo (owner); Received = interlocutores.

COLOR_SENT_START = "#c0392b"
COLOR_SENT_END = "#e74c3c"
COLOR_RECEIVED_START = "#2980b9"
COLOR_RECEIVED_END = "#3498db"
COLOR_SENT_DARK_START = "#922b21"
COLOR_SENT_DARK_END = "#c0392b"
COLOR_RECEIVED_DARK_START = "#1a3a6b"
COLOR_RECEIVED_DARK_END = "#2a5a9b"
COLOR_SENT_LABEL = "#9b1c1c"
COLOR_RECEIVED_LABEL = "#075985"
...
RE_SUBSCRIPTION_TYPE = re.compile(
    r'Subscription Event.*?Type<div class="m"><div>(subscribe|unsubscribe)', re.DOTALL
)
RE_SUBSCRIPTION_USERS = re.compile(
    r'Subscription Event.*?Users<div class="m"><div>(.*?)(?:</div>)', re.DOTALL
)
...
RE_URL_IN_BODY = re.compile(
    r'https?://(?:[-\w.])+(?:\.[a-zA-Z]{2,})+(?:/[^\s<>"\']*)?', re.IGNORECASE
)
