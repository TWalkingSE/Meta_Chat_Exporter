...
from meta_chat_exporter.advanced_stats_schema import ADVANCED_STATS_FAMILIES, build_advanced_section
...
def _csv_cell(value):
    """Converte um valor para célula CSV.

    Estruturas aninhadas (dict/list) são serializadas como JSON compacto para
    caber em uma única célula; ``None`` vira string vazia; escalares passam
    inalterados.
    """
    if value is None:
        return ""
...
def _project_row(source: dict, campos: tuple[str, ...]) -> dict:
    """Projeta um dicionário sobre as colunas declaradas no esquema.

    Garante que a linha contenha exatamente as colunas ``campos`` (mesmos nomes
    do JSON), serializando estruturas aninhadas e omitindo chaves extras.
    """
    return {campo: _csv_cell(source.get(campo)) for campo in campos}


def _flatten_advanced_family(data, campos: tuple[str, ...]) -> list[dict]:
    """Achata a estrutura de uma família de métrica avançada em linhas CSV.

    Regras de achatamento (R29.2):
    - lista no topo -> cada item vira uma linha;
    - dicionário só com escalares -> uma única linha;
    - dicionário com contêineres (listas/dicts) -> cada contêiner é expandido em
      linhas, mesclando os escalares do topo. Dicionários ``periodo -> contagem``
      usam as colunas "extras" do esquema como par chave/valor. Quando há mais de
      um contêiner, a coluna homônima recebe o nome do contêiner para preservar
      a origem de cada linha.

    Os nomes de coluna vêm sempre de ``campos`` (esquema compartilhado), o que
    garante consistência com a exportação JSON (R29.3).
    """
    rows: list[dict] = []
    ...
    if isinstance(data, list):
        for item in data:
            if isinstance(item, dict):
                rows.append(_project_row(item, campos))
            elif campos:
                rows.append(_project_row({campos[0]: item}, campos))
        return rows

    if isinstance(data, dict):
        escalares = {k: v for k, v in data.items() if not isinstance(v, dict | list | tuple)}
        conteineres = {k: v for k, v in data.items() if isinstance(v, dict | list | tuple)}

        if not conteineres:
            return [_project_row(escalares, campos)]

        nomes_conteiner = set(conteineres)
        # Colunas que não são escalares do topo nem nomes de contêiner servem como
        # destino dos itens expandidos (ex.: par "periodo"/"contagem").
        extras = [c for c in campos if c not in escalares and c not in nomes_conteiner]
        marcar_origem = len(conteineres) > 1

        for chave, conteiner in conteineres.items():
            if isinstance(conteiner, list):
                for item in conteiner:
                    ...
                    if isinstance(item, dict):
                        ...
                        base[chave] = item
                    rows.append(_project_row(base, campos))
            elif isinstance(conteiner, dict):  # ex.: por_periodo -> {"2024-01": 5}
                for sub_chave, sub_valor in conteiner.items():
                    base = dict(escalares)
                    if marcar_origem:
                        base[chave] = chave
                    if isinstance(sub_valor, dict):
                        base.update(sub_valor)
                    elif len(extras) >= 2:
                        base[extras[0]] = sub_chave
                        base[extras[1]] = sub_valor
                    elif extras:
                        base[extras[0]] = sub_valor
                    else:
                        base[chave] = sub_valor
                    rows.append(_project_row(base, campos))
        return rows

    # Valor escalar isolado vira uma única linha na primeira coluna.
    if campos:
        return [_project_row({campos[0]: data}, campos)]
    return rows
...
class JSONExporter:
    ...
    def export(self, output_path: Path, include_stats: bool = True) -> Path:
        ...
        if include_stats:
            ...
            data["estatisticas"] = all_stats
            # R29.1/R29.3: incluir as métricas avançadas (R7-R18, R22, R26) em uma
            # subseção dedicada, usando o esquema de nomes de campos compartilhado
            # com a exportação CSV. Famílias ainda não calculadas são omitidas.
            avancadas = build_advanced_section(all_stats)
            if avancadas:
                data["estatisticas"]["avancadas"] = avancadas
            ...
            data["outras_categorias"] = [
                {
                    ...
                    "registros": [{"entradas": rec.entries} for rec in cat.records],
                }
    ...
    def _thread_to_dict(self, thread: Thread) -> dict:
        ...
        participants = [{"nome": p[0], "plataforma": p[1], "id": p[2]} for p in thread.participants]
                ...
                if fieldnames_cat:
                    with open(cat_path, "w", encoding="utf-8-sig", newline="") as f_cat:
                        cat_writer = csv.DictWriter(
                            f_cat, fieldnames=fieldnames_cat, extrasaction="ignore"
                        )
                        cat_writer.writeheader()
                        for rec in cat.records:
                            for entry in rec.entries:
                                cat_writer.writerow(
                                    {k: _sanitize_csv_value(v) for k, v in entry.items()}
                                )
        ...
        if participants:
            ...
            with open(output_path, "w", encoding="utf-8-sig", newline="") as f:
                ...
                for p in participants:
                    writer.writerow({k: _sanitize_csv_value(v) for k, v in p.items()})

        logger.info("Estatísticas CSV exportadas: %s", output_path)
        return output_path

    def export_advanced_stats(self, output_path: Path) -> list[Path]:
        """Exporta as métricas avançadas (R7-R18, R22, R26) em arquivos CSV.

        Gera um arquivo CSV por família de métrica avançada presente em
        ``generate_all()``, derivando o nome a partir de ``output_path`` (stem +
        ``json_key`` da família). As colunas vêm do esquema compartilhado
        ``ADVANCED_STATS_FAMILIES``, os mesmos nomes de campos usados na seção
        ``estatisticas.avancadas`` do JSON, garantindo consistência entre os
        formatos (Requirements 29.2, 29.3).

        Famílias ainda não calculadas (ausentes do dicionário de estatísticas)
        são ignoradas de forma defensiva, sem causar erro.

        Args:
            output_path: caminho base; cada família gera ``<stem>_<json_key>.csv``
                no mesmo diretório.

        Returns:
            Lista dos caminhos efetivamente escritos.
        """
        logger.info("Exportando métricas avançadas para CSV: %s", output_path)
        ...
        for family in ADVANCED_STATS_FAMILIES:
            data = all_stats.get(family.source_key)
            if data is None:
                continue  # família ainda não calculada: omitida sem erro

            rows = _flatten_advanced_family(data, family.campos)
            family_filename = f"{output_path.stem}_{family.json_key}.csv"
            family_path = output_path.with_name(family_filename)

            with open(family_path, "w", encoding="utf-8-sig", newline="") as f:
                writer = csv.DictWriter(f, fieldnames=list(family.campos), extrasaction="ignore")
                writer.writeheader()
                for row in rows:
                    writer.writerow({k: _sanitize_csv_value(v) for k, v in row.items()})

            written.append(family_path)
            logger.info("Métrica avançada CSV exportada: %s", family_path)
