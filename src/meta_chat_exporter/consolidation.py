...
def consolidate_threads(all_threads: list[Thread], log_callback=None) -> list[Thread]:
    ...
    source_files_found: set[str] = set()
    ...
    if log_callback:
        log_callback(
            f"📁 Origens: {', '.join(source_files_found) if source_files_found else 'Nenhuma'}"
        )
    ...
    for thread in all_threads:
        if thread.thread_id in threads_dict:
            ...
            for msg in thread.messages:
                ...
                if sig not in existing_sigs:
                    ...
                    if existing_msg and msg.source_file:
                        if (
                            "records" in msg.source_file.lower()
                            and "records" not in existing_msg.source_file.lower()
                        ):
            ...
            existing_participants = {tuple(p) for p in existing.participants}
            ...
            if hasattr(thread, "past_participants") and thread.past_participants:
                existing_past = {tuple(p) for p in existing.past_participants}
