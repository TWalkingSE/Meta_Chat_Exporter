...
DEFAULT_CONFIG = {
    ...
    "dark_mode": True,
    "log_level": "WARNING",
    "session_gap_minutes": 30,  # limiar de sessão/gap (R11, R13)
    "cooling_threshold": 0.5,  # fração de queda sustentada (R14)
    "sentiment_enabled": False,  # análise de sentimento offline (R18)
}
...
class Config:
    ...
    def session_gap_minutes(self) -> int:
        return self._data["session_gap_minutes"]

    @property
    def cooling_threshold(self) -> float:
        return self._data["cooling_threshold"]

    @property
    def sentiment_enabled(self) -> bool:
        return self._data["sentiment_enabled"]
