...
from meta_chat_exporter.generic_parser import GenericCategoryParser
...
from meta_chat_exporter.widgets import GradientProgressBar
...
class _SignalBridge(QObject):
    ...
    def _run(self, fn):
        try:
            ...
            logger.exception("Erro ao executar atualização na thread principal")
...
class ChatExporterApp(QMainWindow):
    ...
    def __init__(self):
        ...
        # Serviços desacoplados da interface (cache e exportação, sem PyQt)
    ...
    def _setup_ui(self):
        ...
        self.search_type.addItems(
            ["Tudo", "Participante", "Mensagens", "Chamadas", "Links", "Mídias"]
        )
    ...
    def _add_log(self, message: str):
        ...
        QTimer.singleShot(50, _scroll_log_to_bottom)
    ...
    def _open_transcription_dialog(self):
        ...
        try:
            ...
            logger.exception("Erro ao preparar transcrição")
            QMessageBox.critical(self, "Erro na transcrição", f"Erro ao preparar transcrição:\n{e}")
            return
        ...
        self.status_label.setText("⏳ Verificando Whisper/PyTorch...")
        self._add_log("🎙️ Verificando ambiente de transcrição em processo isolado...")
        available, msg, gpu_info = check_transcription_environment()
        if not available:
            self.status_label.setText("⚠️ Transcrição indisponível")
            first_line = msg.splitlines()[0] if msg else "erro"
            self._add_log(f"❌ Transcrição indisponível: {first_line}")
            QMessageBox.warning(self, "Whisper não disponível", msg)
            return
        self.status_label.setText("✅ Ambiente de transcrição verificado")
        ...
        gpu_frame.setStyleSheet(
            f"background-color: {self.colors['bg_dark']}; border-radius: 10px; padding: 12px;"
        )
        ...
        ext_str = ", ".join(
            f"{count}x {ext}" for ext, count in sorted(ext_counts.items(), key=lambda x: -x[1])
        )
        ...
        audio_info = QLabel(
            f"📁 {len(audio_files)} arquivo(s) de áudio encontrado(s)\n   {ext_str}"
        )
        ...
        recommended = str(gpu_info.get("recommended_model", "tiny"))
        if not gpu_info.get("available"):
            recommended = "tiny"
        model_options = [f"{info['label']} — {info['desc']}" for info in WHISPER_MODELS.values()]
        model_keys = list(WHISPER_MODELS.keys())
        model_display_to_key = dict(zip(model_options, model_keys, strict=False))
        if recommended not in model_keys:
            recommended = "tiny"
        ...
        device_options = {
            "Automático": "auto",
            "GPU (Placa de Vídeo / CUDA)": "cuda",
            "CPU (Apenas Processador)": "cpu",
        }
        ...
        prog_frame.setStyleSheet(
            f"background-color: {self.colors['bg_dark']}; border-radius: 10px; padding: 10px;"
        )
        ...
        transcription_state = {"running": False, "closed": False}
        ...
        def set_cancel_callback(callback):
            try:
                ...
                pass
            btn_cancel.clicked.connect(callback)

        def invoke_if_open(fn):
            def _guarded():
                if not transcription_state["closed"]:
                    fn()

            self._bridge.invoke(_guarded)

        def allow_dialog_close():
            transcription_state["running"] = False
            btn_cancel.setEnabled(True)
            btn_cancel.setText("Fechar")
            btn_cancel.setStyleSheet("")
            set_cancel_callback(dialog.close)

        def on_dialog_close(event):
            if transcription_state["running"]:
                self._cancel_transcription(btn_cancel)
                event.ignore()
                return
            transcription_state["closed"] = True
            event.accept()
        ...
        def on_start():
            ...
            force_retranscribe = force_check.isChecked()
            cache_dir = audio_folder / ".chat_export_cache"
            ...
            set_cancel_callback(lambda: self._cancel_transcription(btn_cancel))

            file_start_time = {"t": time.time()}

            def update_progress(current, total, filename, status):
                def _update(_c=current, _t=total, _f=filename, _s=status):
                    short_name = _f[:40] + "..." if len(_f) > 40 else _f

                    # Durante processamento de um arquivo longo, mostrar barra indeterminada
                    # (pulsando) para indicar que a GUI não travou.
                    if _s in ("iniciando", "processando", "carregando modelo"):
                        progress_bar.setRange(0, 0)
                    else:
                        progress_bar.setRange(0, 100)
                        pct = _c / _t if _t > 0 else 0
                        progress_bar.setValue(int(pct * 100))

                    if _s == "iniciando":
                        progress_label.setText(f"🎙️ Iniciando [{_c}/{_t}] {short_name}...")
                        progress_label.setStyleSheet(f"color: {self.colors['warning']};")
                        file_start_time["t"] = time.time()
                    elif _s == "processando":
                        elapsed = time.time() - file_start_time["t"]
                        progress_label.setText(
                            f"🎙️ Transcrevendo [{_c}/{_t}] {short_name}... ({elapsed:.0f}s)"
                        )
                        progress_label.setStyleSheet(f"color: {self.colors['warning']};")
                    elif _s == "transcrevendo":
                        progress_label.setText(f"[{_c}/{_t}] {short_name}")
                        progress_label.setStyleSheet(f"color: {self.colors['text']};")
                    elif _s == "cache":
                        progress_label.setText(f"📦 Cache [{_c}/{_t}] {short_name}")
                        progress_label.setStyleSheet(f"color: {self.colors['text_dim']};")
                    elif _s == "verificando cache":
                        progress_label.setText(f"🔍 Verificando cache [{_c}/{_t}]...")
                        progress_label.setStyleSheet(f"color: {self.colors['text_dim']};")
                    elif _s == "cancelado":
                        progress_label.setText("⏹️ Transcrição cancelada")
                        progress_label.setStyleSheet(f"color: {self.colors['warning']};")
                    elif _s == "concluído":
                        progress_label.setText(f"✅ Concluído! {_t} arquivo(s) processado(s)")
                        progress_label.setStyleSheet(f"color: {self.colors['success']};")
                    elif _s == "carregando modelo":
                        progress_label.setText("⏳ Carregando modelo Whisper...")
                        progress_label.setStyleSheet(f"color: {self.colors['warning']};")
                    elif _s == "modelo pronto":
                        progress_label.setText("✅ Modelo carregado")
                        progress_label.setStyleSheet(f"color: {self.colors['success']};")

                invoke_if_open(_update)

            # Iniciar transcrição em subprocesso isolado (evita crash nativo
            # do torch/PyTorch dentro do processo PyQt6 no Windows)
            import json as _json

            try:
                proc = start_transcription_subprocess(
                ...
                logger.exception("Erro ao iniciar subprocesso de transcrição")
                transcription_state["running"] = False
                allow_dialog_close()
                progress_label.setText(f"❌ Erro ao iniciar: {e}")
                progress_label.setStyleSheet("color: #ff6666;")
                btn_start.setEnabled(True)
                btn_start.setText("▶️ Tentar Novamente")
                return

            self._transcription_process = proc
                ...
                try:
                    for line in proc.stdout:
                        ...
                        if etype == "log":
                            ...
                            invoke_if_open(lambda m=msg: self._add_log(m))
                        elif etype == "progress":
                            update_progress(
                                event.get("current", 0),
                                event.get("total", 0),
                            ...
                            logger.error("Erro no subprocesso de transcrição: %s", err_msg)

                            def on_error(_msg=err_msg):
                                if self._transcription_process is proc:
                                    self._transcription_process = None
                                allow_dialog_close()
                                progress_label.setText(f"❌ Erro: {_msg}")
                                progress_label.setStyleSheet("color: #ff6666;")
                                btn_start.setEnabled(True)
                                btn_start.setText("▶️ Tentar Novamente")

                            invoke_if_open(on_error)

                    proc.wait()

                    if proc.returncode == 0 and results:
                        count = len(set(results.values()))
                        ...
                        def on_complete():
                            ...
                            self.btn_whisper.setStyleSheet(
                                "background-color: #1a7a7a; border-color: #259090;"
                            )
                            self._add_log(f"🎙️ {count} transcrições de áudio prontas para uso")

                        invoke_if_open(on_complete)
                    elif proc.returncode == 0 and not results:
                        ...
                        def on_empty():
                            ...
                            progress_label.setText("ℹ️ Nenhuma transcrição gerada")
                            progress_label.setStyleSheet(f"color: {self.colors['text_dim']};")

                        invoke_if_open(on_empty)
                    elif proc.returncode != 0 and not results:
                        # stderr está mesclado em stdout (STDOUT); não há pipe stderr.
                        err_msg = f"Subprocesso terminou com código {proc.returncode}"

                        def on_error2():
                            if self._transcription_process is proc:
                                self._transcription_process = None
                            allow_dialog_close()
                            progress_label.setText(f"❌ Erro: {err_msg[:200]}")
                            progress_label.setStyleSheet("color: #ff6666;")
                            btn_start.setEnabled(True)
                            btn_start.setText("▶️ Tentar Novamente")

                        invoke_if_open(on_error2)

                except Exception as e:
                    logger.exception("Erro na thread de leitura do subprocesso")
                    err_msg = str(e)
                    ...
                    def on_error3():
                        if self._transcription_process is proc:
                            self._transcription_process = None
                        allow_dialog_close()
                        progress_label.setText(f"❌ Erro: {err_msg}")
                        progress_label.setStyleSheet("color: #ff6666;")
                        btn_start.setEnabled(True)
                        btn_start.setText("▶️ Tentar Novamente")

                    invoke_if_open(on_error3)
                finally:
    ...
    def _inject_transcriptions_dialog(self):
        ...
        html_file, _ = QFileDialog.getOpenFileName(
            self,
            "Selecionar HTML exportado para injetar transcrições",
        ...
        try:
            from meta_chat_exporter.inject_transcriptions import inject_transcriptions_into_html
    ...
    def _select_folder(self):
        ...
        html_files = discover_source_htmls(self.base_dir)
    ...
    def _load_files(self, html_files):
        ...
        for i, html_file in enumerate(html_files):
            try:
                ...
                if file_cached:
                    ...
                    if total_files > 0:
                        self._bridge.invoke(
                            lambda v=(i + 1) / total_files: self._update_progress(v)
                        )
                    continue
                ...
                self._bridge.invoke(
                    lambda e=e, f=html_file: self._add_log(f"❌ Erro em {f.name}: {e}")
                )
        ...
        self._bridge.invoke(
            lambda: self._add_log("📸 Buscando mídias do perfil (fotos, vídeos, stories)...")
        )
            ...
            try:
                ...
                for gcat in generic_cats:
                    existing = next(
                        (
                            c
                            for c in combined_media.generic_categories
                            if c.category_id == gcat.category_id
                        ),
                        None,
                    )
                    if existing:
                ...
                logger.warning(
                    "Erro ao parsear mídias/categorias genéricas de %s: %s", html_file, e
                )
        ...
        if transcription_cache_file.exists() and not self.transcriptions:
            from meta_chat_exporter.transcriber import TranscriptionCache
            ...
            if cached_transcriptions:
                ...
                count = len(set(cached_transcriptions.values()))
                self._bridge.invoke(
                    lambda c=count: self._add_log(f"🎙️ {c} transcrições carregadas do cache")
                )
    ...
    def _is_owner(self, participant: tuple) -> bool:
        ...
        return user_id == self.owner_id or username.lower() == self.owner_username.lower()
    ...
    def _clear_search(self):
        ...
        if self.threads:
            ...
            self.status_label.setText(
                f"✅ {len(self.threads)} conversas • {total_msgs:,} mensagens"
            )
        self._update_threads_list()
    ...
    def _export_thread(self, thread: Thread, open_browser=True):
        ...
        try:
            ...
            if redact:
                self._add_log("🔒 Modo redigido ativo — nomes e números serão ocultados.")

            output_path, filename = self.export_service.export_thread_html(
                thread,
                self.owner_username,
                self.owner_id,
                self.transcriptions,
            ...
            self._add_log("🔏 Manifesto de custódia (SHA-256) gravado na pasta de exportação")
    ...
    def _export_all(self):
        ...
        thread = threading.Thread(
            target=self._export_all_background, args=(threads_to_export, is_filtered)
        )
        thread.start()
    ...
    def _export_all_background(self, threads_to_export, is_filtered=False):
        ...
        try:
            ...
            # Phase 8.3: Modo redigido evita mutação no estado da UI (cópia feita no serviço)
            redact = getattr(self, "redact_mode", False)
            if redact:
                self._bridge.invoke(
                    lambda: self._add_log(
            ...
            output_path, filename = self.export_service.export_all_html(
                threads_to_export,
                self.owner_username,
                self.owner_id,
                self.transcriptions,
            ...
            def _finish():
                ...
                self._add_log("🔏 Manifesto de custódia (SHA-256) gravado na pasta de exportação")
                self.status_label.setText(
                    f"✅ HTML gerado com {len(threads_to_export)} conversas{filter_label}!"
                )
                self.btn_select.setEnabled(True)
                self.btn_export_all.setEnabled(True)
            ...
            def _error():
                self._add_log(f"❌ Erro: {err_msg}")
                self.status_label.setText("❌ Erro ao exportar")
                self.btn_select.setEnabled(True)
                self.btn_export_all.setEnabled(True)
                QMessageBox.critical(self, "Erro", f"Erro ao exportar: {err_msg}")
    ...
    def _export_json(self):
        ...
        def _background():
            try:
                output_path, filename = self.export_service.export_json(
                    threads_to_export,
                    self.owner_username,
                    self.owner_id,
                    self.base_dir,
                ...
                def _error():
                    self._add_log(f"❌ Erro JSON: {err_msg}")
                    self.btn_export_json.setEnabled(True)
                    QMessageBox.critical(self, "Erro", f"Erro ao exportar JSON: {err_msg}")
    ...
    def _export_csv(self):
        ...
        def _background():
            try:
                output_path, filename, stats_path, stats_filename = self.export_service.export_csv(
                    threads_to_export,
                    self.owner_username,
                    self.owner_id,
                    self.base_dir,
                ...
                def _error():
                    self._add_log(f"❌ Erro CSV: {err_msg}")
                    self.btn_export_csv.setEnabled(True)
                    QMessageBox.critical(self, "Erro", f"Erro ao exportar CSV: {err_msg}")
...
def main():
    ...
    # Ativar faulthandler para logar crashes nativos (segfault, etc.)
    fault_fh = open(log_file, "a", encoding="utf-8")  # noqa: SIM115
    faulthandler.enable(fault_fh)

    logger.info("=" * 60)
    logger.info("Chat Exporter v5.4 (PyQt6) iniciado")
