...
try:
    from bs4 import BeautifulSoup  # noqa: F401  # importado para checar disponibilidade

    BS4_AVAILABLE = True
except ImportError:
    BS4_AVAILABLE = False

from meta_chat_exporter.models import GenericCategory, GenericRecord
...
_RE_KV_PAIR = re.compile(
    r'([^<>]+?)(?:</div>|<br\s*/?>|<\/?a[^>]*>|\s)*<div class="m"><div>(.*?)</div>', re.DOTALL
)
...
class GenericCategoryParser:
    ...
    def parse(self) -> list[GenericCategory]:
        ...
        for i, match in enumerate(matches):
            ...
            if records:
                categories.append(
                    GenericCategory(category_id=cat_id, category_name=cat_name, records=records)
                )
