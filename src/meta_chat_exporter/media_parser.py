...
from meta_chat_exporter.constants import RE_PAGE_BREAK_FULL, get_timezone_offset
from meta_chat_exporter.models import Photo, ProfileMedia, Story, Video
from meta_chat_exporter.utils import is_safe_relative_path

logger = logging.getLogger(__name__)

# Regex para extrair campos das seções de mídia
RE_MEDIA_ID_FIELD = re.compile(r'Id<div class="m"><div>(\d{10,25})<div class="p">', re.DOTALL)
...
RE_MEDIA_PRIVACY = re.compile(r'Privacy Setting<div class="m"><div>([^<]+)', re.DOTALL)
RE_MEDIA_OWNER = re.compile(r'Owner<div class="m"><div>([^<]+)', re.DOTALL)
RE_MEDIA_LINKED = re.compile(r'Linked Media File:<div class="m"><div>([^<]+)', re.DOTALL)
RE_MEDIA_LIKE_COUNT = re.compile(r'Like Count<div class="m"><div>(\d+)', re.DOTALL)
...
RE_MEDIA_LOCATION_ADDR = re.compile(r'Address<div class="m"><div>([^<]+)', re.DOTALL)
RE_MEDIA_CAROUSEL = re.compile(r'Carousel Id<div class="m"><div>([^<]+)', re.DOTALL)
RE_MEDIA_FILTER = re.compile(r'Filter<div class="m"><div>([^<]+)', re.DOTALL)
RE_MEDIA_PUBLISHED = re.compile(r'Is Published<div class="m"><div>([^<]+)', re.DOTALL)
RE_MEDIA_SOURCE = re.compile(r'Source<div class="m"><div>([^<]+)', re.DOTALL)
RE_MEDIA_AI = re.compile(r'Ai<div class="m"><div>([^<]+)', re.DOTALL)
RE_MEDIA_IMG_SRC = re.compile(r'<img[^>]+src="([^"]+)"', re.DOTALL)
RE_MEDIA_VIDEO_SRC = re.compile(r'<source\s+src="([^"]+)"\s+type="video/mp4"', re.DOTALL)

# Para dividir entries: photos/videos usam "Linked Media File:", stories usam ID numérico
RE_LINKED_ENTRY_SPLIT = re.compile(r'Linked Media File:<div class="m"><div>[^<]+')
...
class MediaParser:
    ...
    def parse(self) -> ProfileMedia:
        ...
        profile_pics = self._parse_section(
            content, "profile_picture", self._parse_photo_entry, category="Profile Picture"
        )
        if profile_pics:
        ...
        # Live Videos
        live_videos = self._parse_section(
            content, "live_videos", self._parse_video_entry, category="Live Videos"
        )
        if live_videos:
        ...
        archived_live = self._parse_section(
            content,
            "archived_live_videos",
            self._parse_video_entry,
            category="Archived Live Videos",
        )
        if archived_live:
        ...
        # Archived Stories
        stories = self._parse_section(
            content, "archived_stories", self._parse_story_entry, category="Archived Stories"
        )
        ...
        # Unarchived Stories
        unarchived = self._parse_section(
            content, "unarchived_stories", self._parse_story_entry, category="Unarchived Stories"
        )
        if unarchived:
    ...
    def _parse_section(
        self, content: str, section_name: str, parser_func, category: str = ""
    ) -> list:
    ...
    def _parse_common_media_fields(self, entry_html: str, media_type: str) -> dict | None:
        ...
        if not is_safe_relative_path(local_path):
            logger.warning("Path inseguro detectado em %s, ignorando: %s", media_type, local_path)
            return None
    ...
    def _parse_photo_entry(self, entry_html: str) -> Photo | None:
    ...
    def _parse_video_entry(self, entry_html: str) -> Video | None:
    ...
    def _parse_story_entry(self, entry_html: str) -> Story | None:
        ...
        # Rejeitar paths inseguros (traversal "../" ou caminhos absolutos)
        if not is_safe_relative_path(local_path):
            logger.warning("Path inseguro detectado em story, ignorando: %s", local_path)
            return None
