...
from meta_chat_exporter.constants import (
    MONTHS,
    RE_INSTAGRAM_REEL_ID,
    RE_MENTION,
    RE_URL_IN_BODY,
    RE_VOICE_MESSAGE_BODY,
)
from meta_chat_exporter.models import Attachment, Message
from meta_chat_exporter.utils import is_safe_relative_path, is_safe_url, translate_message
...
def _enrich_body_html(escaped_body: str) -> str:
    ...
    for p in parts:
        if p.startswith("<a "):
            ...
            p = RE_MENTION.sub(lambda m: f'<span class="mention">@{m.group(1)}</span>', p)
            out_parts.append(p)
...
class BaseHTMLGenerator:
    ...
    def __init__(
        self, owner_username: str, owner_id: str, transcriptions: dict | None = None
    ):
    ...
    def _generate_author_html(self, msg: Message, is_sent: bool) -> str:
        author_class = "message-author-sent" if is_sent else "message-author-received"
        return f'<div class="message-author {author_class}">{html.escape(msg.author)}</div>'

    def _generate_audio_speed_controls(self, audio_id: str) -> str:
        safe_audio_id = html.escape(audio_id, quote=True)
        return (
            f'<div class="audio-speed" data-audio-id="{safe_audio_id}" role="group" '
            'aria-label="Velocidade de reprodução">'
            '<button type="button" class="active" data-playback-rate="1" aria-pressed="true">1x</button>'
            '<button type="button" data-playback-rate="1.5" aria-pressed="false">1.5x</button>'
            '<button type="button" data-playback-rate="2" aria-pressed="false">2x</button>'
            "</div>"
        )
    ...
    def _generate_attachment(self, att: Attachment) -> str:
        ...
        if not is_safe_relative_path(att.local_path):
            logger.warning("Path inseguro detectado, ignorando anexo: %s", att.local_path)
            return ""
    ...
    def _generate_message_content(self, msg: Message) -> str:
        ...
        if msg.removed_by_sender:
            ...
            duration = (
                f" • {msg.call_duration//60}:{msg.call_duration%60:02d}"
                if msg.call_duration > 0
                else ""
            )
                ...
                if RE_VOICE_MESSAGE_BODY.match(translated_body.strip()):
                    content = (
                        ...
                        + "".join(
                            ...
                            for h in (30, 60, 45, 80, 55, 70, 40, 90, 50, 65, 35, 75, 45, 60, 30)
                        )
                        + "</div>"
    ...
    def _generate_share_card(self, share_url: str, share_text: str | None = None) -> str:
        """Gera card rico para shares do Instagram e outros serviços.
        Phase 6.1 — v5.2."""
        url = share_url.strip()
        text = (share_text or "").strip()
        if not is_safe_url(url):
            # Scheme não permitido (javascript:, data:, etc.): exibir texto sem href clicável
            text_safe = html.escape(text or url or "Link")
            url_safe = html.escape(url)
            return (
                f'<div class="share-link">'
                f'<span>🔗 {text_safe}</span>'
        ...
        if reel_match:
            ...
            caption_html = (
                f'<div class="share-card-caption">{html.escape(text)}</div>' if text else ""
            )
            return (
